...
module IceBagEvaluation
  ...
  def self.ice_cube_acc_power(file_path)
    ...
    IO
      .foreach(file_path)
      .map { |game_record| record_power_set(game_record) }
      .sum
  end
  ...
  def self.record_power_set(game_record)
    ...
    color_to_min_count = INITIAL_COLOR_TO_MIN_COUNT.dup

    game_sets.each do |game_set|
      color_to_min_count = find_min_color_for_cubes(game_set, color_to_min_count)
    end

    color_to_min_count[:red] * color_to_min_count[:green] * color_to_min_count[:blue]
  end
